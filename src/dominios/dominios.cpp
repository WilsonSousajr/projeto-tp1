...
void Codigo::validar(const string &valor) const {
  ...
  for (char c : valor) {
    if (!(islower(c) || isdigit(c))) {
      throw invalid_argument("Codigo deve conter apenas letras minusculas ou digitos.");
    }
  }
};
...
void Data::validar(const string &valor) const {
  ...
  size_t secondDash = valor.find('-', firstDash == string::npos ? 0 : firstDash + 1);
  if (firstDash == string::npos || secondDash == string::npos || secondDash == valor.size() - 1) {
  ...
  // Mes
  static const string mesesValidos[] = {"JAN","FEV","MAR","ABR","MAI","JUN","JUL","AGO","SET","OUT","NOV","DEZ"};
  bool mesOk = false;
  int mesNumero = -1;
  for (int i = 0; i < 12; ++i) {
  ...
  case 4: case 6: case 9: case 11:
    diasNoMes = 30; break;
  case 2:
    diasNoMes = isLeap(ano) ? 29 : 28; break;
  default:
    diasNoMes = 31;
  }
  if (dia > diasNoMes) {
...
void Email::validar(const string &valor) const {
  ...
  for (size_t i = 0; i < local.size(); ++i) {
    ...
    if ((c == '.' || c == '-') ) {
      if (i + 1 >= local.size() || !(ehLetra(local[i+1]) || ehDigito(local[i+1]))) {
        throw invalid_argument("Ponto ou hifen deve ser seguido de letra(s) ou digito(s).");
      }
    }
  }
  ...
  while (start < dominio.size()) {
    ...
    if (parte.front() == '-' || parte.back() == '-') {
      throw invalid_argument("Parte do dominio nao pode comecar ou terminar com hifen.");
    }
    for (char c : parte) {
...
void Endereco::validar(const string &valor) const {
  ...
  if (primeiro == ',' || primeiro == '.' || isspace(static_cast<unsigned char>(primeiro)) ||
      ultimo == ',' || ultimo == '.' || isspace(static_cast<unsigned char>(ultimo))) {
    throw invalid_argument("Endereco nao pode comecar ou terminar com virgula, ponto ou espaco.");
  }

  for (size_t i = 0; i < valor.size(); ++i) {
    char c = valor[i];
    if (!(ehLetra(c) || ehDigito(c) || c == ',' || c == '.' || isspace(static_cast<unsigned char>(c)))) {
    ...
    if (c == ',') {
      ...
      char prox = valor[i+1];
      if (prox == ',' || prox == '.') {
        throw invalid_argument("Virgula nao pode ser seguida de virgula ou ponto.");
      }
    } else if (c == '.') {
      if (i + 1 >= valor.size()) {
      ...
      char prox = valor[i+1];
      if (prox == ',' || prox == '.') {
        throw invalid_argument("Ponto nao pode ser seguido de virgula ou ponto.");
      }
...
void Senha::validar(const string &valor) const {
  ...
  for (size_t i = 0; i < valor.size(); ++i) {
    ...
    if (i > 0) {
      char p = valor[i-1];
      if ((ehMinuscula(p)||ehMaiuscula(p)) && (ehMinuscula(c)||ehMaiuscula(c))) {
    ...
    if (ehMinuscula(c)) temMin = true;
    if (ehMaiuscula(c)) temMai = true;
    if (ehDigito(c)) temDig = true;
    if (ehEspecial(c)) temEsp = true;
  }
  if (!(temMin && temMai && temDig && temEsp)) {
    throw invalid_argument("Senha deve conter minuscula, maiuscula, digito e especial.");
  }
};
